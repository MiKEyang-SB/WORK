...
class Arguments(tap.Tap):
    root_dir: Path = '/home/server/data/RLBench_dataset/val'


def main(root_dir, task):
    variations = os.listdir(f'{root_dir}/{task}/all_variations/episodes')
    #[episode0, episode1, ...]
    if '.DS_Store' in variations:
        variations.remove('.DS_Store')
    seen_variations = {}
    for variation in variations:
        num = int(variation.replace('episode', ''))#[0, 1, ...]
        ...
        #./task/variationepisode0/episodes
        if variation not in seen_variations.keys():
            seen_variations[variation] = [num] #此变体的第几个variation
        else:
