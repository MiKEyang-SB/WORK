...
def _parse_phase_ratio(val, default=(0.02, 0.08, 0.90)):
    if val is None:
        return default
    if isinstance(val, (tuple, list)):
        return tuple(val)
    # 允许字符串形式 "(0.02, 0.08, 0.9)"
    try:
        return tuple(ast.literal_eval(val))
    except Exception:
        return default

def warmup_tristage(global_step, warmup_steps, num_train_steps,
                    init_lr_scale=0.1, final_lr_scale=1e-6,
                    phase_ratio=(0.02, 0.08, 0.90), decay_type="cosine"):
    """
    返回相对峰值 LR 的倍率（multiplier），范围约在 [final_lr_scale, 1.0]
    三阶段：warmup -> flat -> decay
    - warmup: 线性从 init_lr_scale 升到 1.0
    - flat:   保持 1.0
    - decay:  线性或余弦，从 1.0 降到 final_lr_scale
    """
    # 允许 warmup_steps 外部给定；否则按 phase_ratio[0] 推算
    if warmup_steps is None or warmup_steps <= 0:
        warmup_steps = max(1, int(num_train_steps * float(phase_ratio[0])))
    flat_steps   = max(0, int(num_train_steps * float(phase_ratio[1])))
    decay_steps  = max(1, num_train_steps - warmup_steps - flat_steps)

    gs = int(global_step)

    if gs < warmup_steps:
        # 线性升温
        t = gs / float(max(1, warmup_steps))
        return float(init_lr_scale + (1.0 - init_lr_scale) * t)

    elif gs < warmup_steps + flat_steps:
        return 1.0

    else:
        t = (gs - warmup_steps - flat_steps) / float(decay_steps)   # 0..1
        if decay_type == "linear":
            return float(1.0 + (final_lr_scale - 1.0) * t)
        else:  # "cosine"
            # 从 1.0 余弦衰减到 final_lr_scale
            return float(final_lr_scale + (1.0 - final_lr_scale) * 0.5 * (1.0 + math.cos(math.pi * t)))
...
def get_lr_sched(global_step, opts):
    ...
    elif opts.lr_sched == 'tri_stage':
        lr_decay_fn = warmup_tristage
        # 读入三阶段参数，提供合理默认
        init_lr_scale   = getattr(opts, "init_lr_scale", 0.1)
        final_lr_scale  = getattr(opts, "final_lr_scale", 1e-6)
        phase_ratio     = _parse_phase_ratio(getattr(opts, "phase_ratio", "(0.02, 0.08, 0.90)"))
        decay_type      = getattr(opts, "tri_decay", "cosine")   # or "linear"
        fn_args.extend([init_lr_scale, final_lr_scale, phase_ratio, decay_type])
    else:
